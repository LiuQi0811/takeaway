 ...
 * @author LiuQi
 * @version 1.0
 * @data 2022/12/2 10:37
 *  *  对象映射器:基于jackson将Java对象转为json，或者将json转为Java对象
 *  * 将JSON解析为Java对象的过程称为 [从JSON反序列化Java对象]
 *  * 从Java对象生成JSON的过程称为 [序列化Java对象到JSON]
 */
public class JacksonObjectMapper extends ObjectMapper {
    public static final String DEFAULT_DATE_FORMAT = "yyyy-MM-dd"; // 日期格式 年月日
    public static final String DEFAULT_DATE_TIME_FORMAT = "yyyy-MM-dd HH:mm:ss"; // 日期格式 年月日 时分秒
    public static final String DEFAULT_TIME_FORMAT = "HH:mm:ss"; // // 日期格式 时分秒

    // 初始化构造方法
    public JacksonObjectMapper(){
        super(); // 继承父类
        this.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES,false); // 收到未知属性时不报异常
        this.getDeserializationConfig().withFeatures(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);// 反序列化时，属性不存在的兼容处理
        SimpleModule simpleModule = new SimpleModule(); // 初始化序列化对象
